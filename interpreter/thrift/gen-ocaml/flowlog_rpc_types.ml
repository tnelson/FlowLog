...
class queryReply =
object (self)
  val mutable _result : string list list option = None
  ...
  method write (oprot : Protocol.t) =
    ...
    | Some _v -> 
      oprot#writeFieldBegin("result",Protocol.T_LIST,1);
      oprot#writeListBegin(Protocol.T_LIST,List.length _v);
      List.iter (fun _iter34 ->         oprot#writeListBegin(Protocol.T_STRING,List.length _iter34);
...
let rec read_queryReply (iprot : Protocol.t) =
  let _str36 = new queryReply in
    ...
    (try while true do
        ...
        (match _id38 with 
          | 1 -> (if _t37 = Protocol.T_LIST then
              _str36#set_result 
                (let (_etype42,_size39) = iprot#readListBegin in
                  let _con43 = (Array.to_list (Array.init _size39 (fun _ -> 
                    (let (_etype47,_size44) = iprot#readListBegin in
                      let _con48 = (Array.to_list (Array.init _size44 (fun _ -> iprot#readString))) in
                        iprot#readListEnd; _con48)))) in
                    iprot#readListEnd; _con43)
            else
              iprot#skip _t37)
