...
(**************************************************************************************************)
(* For reachability-aware preds, we can assume that the only values fed into the system are
   - constants that appear in the program
   - the event ceiling (a fresh one every step)
   - generated terms (via add)

   This fact means that we don't need to count every program variable---just those generated by built-ins.
   Also note that the startup event, which must always come first, is empty; it has no fields.

   Default length is all events except startup.
   Thus the number of states in the model is max = default + 2 (including one for the empty state)
 *)
