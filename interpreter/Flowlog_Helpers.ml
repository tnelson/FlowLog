...
let get_program_var_counts (p: flowlog_program): vars_count_report =
  fold_left (fun acc cl ->
      ...
      printf "vars generated by builtins: %s\n%s\n%!"
      ...
      let add_counts (m: int StringMap.t) (tlst: term list) (gentlst: term list): int StringMap.t =
          fold_left (fun acc t ->
            ...
            begin
              ...
              let previous = if (StringMap.mem typename m) then (StringMap.find typename m) else 0 in
                (StringMap.add typename (previous+1) m)) m (tlst@gentlst) in
        ...
        match cl.head with
          | FAtom(_,relname,_) when (starts_with relname plus_prefix) || (starts_with relname minus_prefix) ->
            {acc with transition_qvars_ext=add_counts acc.transition_qvars_ext ntqvs_ext builtin_generated_vars_e;
                      transition_qvars_uni=add_counts acc.transition_qvars_uni ntqvs_uni builtin_generated_vars_u;}
          | _ ->
            {acc with action_qvars_ext=add_counts acc.action_qvars_ext ntqvs_ext builtin_generated_vars_e;
                      action_qvars_uni=add_counts acc.action_qvars_uni ntqvs_uni builtin_generated_vars_u})
