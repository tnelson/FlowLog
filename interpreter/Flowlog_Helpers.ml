...
let get_non_trigger_quantified_vars (p: flowlog_program) ?(include_head: bool = true) (cl:clause): (term list * term list) =
  let triggerargs = (match get_trigger p cl with | Some(_,_,args) -> args | None -> []) in
  let to_exclude = if include_head then triggerargs else (get_head_vars cl) @ triggerargs in
  let signed_free = get_terms_with_sign
      (function | TVar(x) as t -> not (mem t to_exclude) | _ -> false) true cl.body in
...
(* Count the number of quantified vars in the program, indexed by type and quantifier
  ...
  CAVEAT: does not include constants

  Use ceilings_only to control whether to count ALL vars or just ceilings per clause
    *)
let get_program_var_counts ?(include_head_vars : bool = true) ?(ceilings_only : bool = true) (p: flowlog_program) : vars_count_report =
  fold_left (fun acc cl ->
      let ntqvs_ext, ntqvs_uni = get_non_trigger_quantified_vars p ~include_head:include_head_vars cl in
      let infers = type_inference_of_vars p TermMap.empty cl in

      printf "\ngetting var counts for clause: %s%!" (string_of_clause cl);
      printf "ntqvs_ext: %s; ntqvs_uni: %s\n%!"
      ...
      let add_or_ceiling (m: int StringMap.t) (tlst: term list) (gentlst: term list): int StringMap.t =
        if ceilings_only then ceiling_counts m tlst gentlst
        else add_counts m tlst gentlst in
        ...
        match cl.head with
          | FAtom(_,relname,_) when (starts_with relname plus_prefix) || (starts_with relname minus_prefix) ->
            {acc with transition_qvars_ext=add_or_ceiling acc.transition_qvars_ext ntqvs_ext builtin_generated_vars_e;
                      transition_qvars_uni=add_or_ceiling acc.transition_qvars_uni ntqvs_uni builtin_generated_vars_u;}
          | _ ->
            {acc with action_qvars_ext=add_or_ceiling acc.action_qvars_ext ntqvs_ext builtin_generated_vars_e;
                      action_qvars_uni=add_or_ceiling acc.action_qvars_uni ntqvs_uni builtin_generated_vars_u})
